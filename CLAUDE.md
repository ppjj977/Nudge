...
- **Admin portal `/admin`** (gated by `ADMIN_EMAIL`): subscriber stats, **All users
  table** with one-click Make Pro / Revoke (`/api/admin/plan`, `setPlanById`) + search,
  Grant Pro by email, promo codes, demo-account button, tester invite links, waitlist.
